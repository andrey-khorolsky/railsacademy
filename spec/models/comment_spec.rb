...
RSpec.describe Comment, type: :model do
  ...
  context "find some comments" do
    it { expect(Comment.getLastComments 1).to eq Comment.where('post_id = ?', 1).order(created_at: :desc).limit(count)}
    it { expect(Comment.getLastComments 2).to eq Comment.where('post_id = ?', 2).order(created_at: :desc).limit(count)}
  end
